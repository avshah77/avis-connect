...
from connect.core import registry
from connect.db import get_db
from connect.models import ConnectorConfig, Operation, Project, PushLog
...
def log_detail(log_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "logs/detail.html", {
        "log":          log,
...
def connectors_list(request: Request, db: Session = Depends(get_db)):
    ...
    rows = (db.query(ConnectorConfig)
            ...
            .all())
    from connect.models import Credential
    ...
    return templates.TemplateResponse(request, "connectors/list.html", {
        "connectors": connectors,
    })
...
def connector_add_form(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "connectors/add.html", {
        "projects":         projects,
        "plugin_names":     plugin_names,
...
def connector_edit_form(connector_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "connectors/edit.html", {
        "connector": {
            "id":             c.id,
            "project_id":     c.project_id,
            "project_slug":   c.project.slug,
